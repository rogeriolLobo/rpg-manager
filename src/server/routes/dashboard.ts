...
dashboardRoutes.get('/', async (c) => {
  ...
  const rpgs = (rpgRows.results as Array<Record<string, unknown>>).map((row) => {
    const candidate: RecommendationCandidate = { title: String(row.title), readingStatus: row.reading_status as RecommendationCandidate['readingStatus'],
      ...
      hasPlayGroup: Boolean(row.play_group_name || String(row.play_group_notes ?? '').trim()), tableStatus: row.table_status as RecommendationCandidate['tableStatus'] };
    return { id: row.id, ...candidate, playGroupNotes: row.play_group_notes, playGroupName: row.play_group_name, plannedPlayDate: row.planned_play_date,
      subgenreName: row.subgenre_name, score: calculateRpgRecommendationScore(candidate), readiness: calculateRpgReadiness(candidate), nextAction: calculateRpgNextAction(candidate) };
