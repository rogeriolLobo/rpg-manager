...
export function DashboardPage(){const [data,setData]=useState<DashboardData>();useEffect(()=>{void api<DashboardData>('/dashboard').then(setData);},[]);if(!data)return <Loading/>;const metricEntries=[['Total de RPGs',data.metrics.total],['Já lidos',data.metrics.read],['Já jogados',data.metrics.played],['Quero jogar',data.metrics.wants],['Ainda não lidos',data.metrics.unread],['Mesas agendadas',data.metrics.scheduled],['Leitura concluída',`${data.metrics.readingCompletion}%`]];
