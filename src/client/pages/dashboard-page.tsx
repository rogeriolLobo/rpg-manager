...
const quickAccessLinks = [
  ...
  { to:'/app/sheets', label:'Fichas', description:'Modelos e templates', Icon:ScrollText },
  { to:'/app/compendium', label:'Compêndio', description:'Consulta rápida', Icon:BookMarked },
] as const;
export function DashboardPage(){const resource=useResource<DashboardData>('/dashboard');if(resource.status!=='success')return <ResourceFallback state={resource} onRetry={resource.reload}/>;const data=resource.data;const metricEntries=[['Total de RPGs',data.metrics.total],['Já lidos',data.metrics.read],['Já jogados',data.metrics.played],['Quero jogar',data.metrics.wants],['Ainda não lidos',data.metrics.unread],['Mesas agendadas',data.metrics.scheduled],['Leitura concluída',`${data.metrics.readingCompletion}%`]];
  return <div className="page"><PageHeader eyebrow="Visão geral" title="Painel do aventureiro" description="Sua coleção e o caminho mais curto até a próxima mesa." action={<div className="button-row"><QuickIdeaButton/><Link className="primary-button link-button" to="/app/library/new">Adicionar RPG</Link></div>}/><section className="panel quick-access" aria-labelledby="quick-access-heading"><div className="section-heading"><div><h2 id="quick-access-heading">Acesso rápido</h2><p className="section-note">Comece pelas áreas principais sem precisar conhecer rotas internas.</p></div></div><nav className="quick-access-grid" aria-label="Acesso rápido">{quickAccessLinks.map(({to,label,description,Icon})=><Link to={to} key={to}><Icon size={20}/><span><strong>{label}</strong><small>{description}</small></span></Link>)}{data.campaigns[0]&&<Link to={`/app/campaigns/${data.campaigns[0].id}/vtt`}><Swords size={20}/><span><strong>Mesa do Mestre</strong><small>{data.campaigns[0].name}</small></span></Link>}</nav></section><section className="metric-grid">{metricEntries.map(([label,value],index)=>{const Icon=metricIcons[index];return <article className="metric-card" key={label}><Icon/><span>{label}</span><strong>{value}</strong></article>})}</section>
