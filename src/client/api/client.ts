...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  const body = await response.json() as { error?: { code: string; message: string; fields?: Record<string,string[]> } } & T;
  if (!response.ok && body.error) throw new ClientApiError(body.error.code, body.error.message, response.status, body.error.fields);
  if (!response.ok) throw new ClientApiError('HTTP_ERROR', 'Não foi possível concluir a operação.', response.status);
  return body;
}
