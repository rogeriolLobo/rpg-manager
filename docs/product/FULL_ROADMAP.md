...
| F-015 | Backup/Restore JSON completo | `NOT_STARTED` — achado real da auditoria BATCH5, primeiro item desta execução |
...
|---|---|---|---|---|---|---|---|---|---|---|
| F-015 | Backup/Restore completo, versionado (`schemaVersion`) | Só export parcial hoje (Library completo; Worlds/Vault só linha-base, sem campos especializados/Journal/Wiki/Relations/Maps/External Resources/Revisions) | Sim — `CLAUDE.md` §22 do pedido original de finalização, achado real desta sessão | Nenhuma (todos os domínios já existem) | Médio — restore mal feito pode sobrescrever dados reais | Não (read/write de dados existentes, sem schema novo) | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` |
| F-016 | Social — amizades (busca, pedido, aceitar, recusar, cancelar, remover, bloquear) | Ausente | Sim — `CLAUDE.md` §32 (design explícito: amigos→bibliotecas→interesses→grupos→propostas→campanhas), adiado até estabilização do 1.0 (agora revogado) | F-015 (backup deve cobrir dados sociais desde o início) | Alto — superfície de IDOR/privacidade nova | Sim — tabelas novas (`friendships`, `blocks`) | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` |
| F-017 | Social + Biblioteca (visão parcial da Library de amigos, RPGs em comum, interesse social ≠ campo pessoal "Quero jogar") | Ausente | Sim — `CLAUDE.md` §32 | F-016 | Alto — vazamento de notas/prioridade privadas se a política de visibilidade não for explícita | Sim — coluna/tabela de interesse social separada do campo pessoal existente | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` |
| F-018 | Social + Grupos/Campanhas (convidar amigo, propostas de mesa, guests) | Ausente | Sim — `CLAUDE.md` §32 | F-016 | Médio — remoção de membro não pode apagar histórico (sessions/attendance já são append-only) | Sim — tabela de convites/propostas | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` |
| F-019 | Notificações in-app (D1, zero-cost) | Ausente | Sim — decorre diretamente de F-016/F-018 (pedido de amizade, convite) | F-016, F-018 | Baixo — CRUD simples, sem realtime obrigatório | Sim — tabela `notifications` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` |
