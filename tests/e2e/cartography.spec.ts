...
test("World → Cartografia → criar mapa → adicionar pin → ver e remover", async ({ page }) => {
  // Fluxo multioperação real (RPG, World, mapa e pin); margem local para Worker/D1.
  ...
  await expect(page.getByRole("heading", { name: "Sistema de Mapas" })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByRole("heading", { name: "Terras Cartografadas" })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByRole("heading", { name: "Mapas do World" })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByText("Mapa da Capital")).toBeVisible({ timeout: 30_000 });
  await page.getByText("Mapa da Capital").click();

  await expect(page.getByRole("heading", { name: "Mapa da Capital" })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByRole("button", { name: "Pin: Praça Central" })).toHaveCount(0, { timeout: 30_000 });
});
