...
test("VTT ao vivo (F-031): jogador acessa o link direto, vê a cena/token, e o combate aparece após o mestre iniciar", async ({ page, browser }) => {
  ...
  try {
    ...
    // Timeout folgado: primeira conexão de WebSocket/Durable Object desta Campaign soma
    // latência real de D1 + Durable Object + coordenação do runtime local — bem acima do
    // timeout padrão de asserção do projeto (10s), sem ser um `waitForTimeout` cego.
