...
test("Campanhas: atalho 'Nova mesa única' pré-seleciona o Formato One-Shot e a mesa criada aparece marcada na listagem", async ({ page }) => {
  // Jornada completa com seis gravações reais no Worker/D1; a margem é exclusiva deste
  // cenário e não altera os limites globais ou suas asserções funcionais.
  ...
  await expect(page.getByRole("heading", { name: `RPG OneShot ${suffix}` })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByRole("heading", { name: `Mesa Única ${suffix}` })).toBeVisible({ timeout: 30_000 });
  await expect(page.getByText("One-Shot").first()).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByLabel("Jogador Jogador Convidado")).toHaveValue("Jogador Convidado", { timeout: 30_000 });
  ...
  await page.getByRole("link", { name: "Mesa do Mestre", exact: true }).click();
  ...
  await expect(page.getByLabel("Nome da campanha")).toHaveValue(`Mesa Única ${suffix}`, { timeout: 30_000 });
